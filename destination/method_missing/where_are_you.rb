...
live_loop :bazz, sync: :thing do
  ...
  b = (ring  #1.75       0.875 per line         # 0.4375
       ...
       [chord(:Fs2, :m,  invert: 1), 0.75],     [[], 1.0], [[], 0.25]
       ).tick(:n2)
  puts note_inspect(b[0])
  with_transpose 0 do
    #if b[0].length > 1
    synth :fm, note: b[0][0], amp: 1.0, decay: b[1]*2 , attack: 0.001
    #end
    if b[0].length == 1
      #synth :dark_sea_horn, note: :Fs2, amp: 1.0, decay: 8*b[1] , attack: 0.001
    end
    synth :tb303, note: b[0], amp: 2.0, decay: b[1] , attack: 0.001, cutoff: 60
    with_fx(:slicer, phase: 0.5, probability: 0, invert_wave: 1) do
      synth :dsaw, note: b[0], amp: 1.0, decay: b[1]*4 , attack: 0.001, cutoff: 65
    end
    ...
    l = (ring 1.0, 0.5, 0.5, 0.5, 0.5, 1.0, 0.5).tick/4.0
      ...
      with_fx :distortion, mix: 0.2, distort: (range 0.1, 0.7, 0.1).tick(:r) do
        with_fx :bitcrusher, bits: (range 4,16,1).tick(:b), mix: 0.2 do
          # synth :dsaw, note: n, release: l, cutoff: (rrand 80,90), amp: 0.6*0.3
        end
      end
