  ...
  with_fx :distortion, mix: 0.7 do
    with_fx :wobble, phase: 16, invert_wave: 0  do
      with_fx :slicer, phase: 0.25, probability: 0.5  do
        synth :dark_ambience, note: chord(:Gs6, :m13), amp: 5, decay: 8
      end
    end
  end
  sleep 16
...
live_loop :fourth_voice, sync: :thing do
  sleep 4
  4.times{
    synth :plucked, note: (ring :E5, :E4).tick(:d), amp: 1.5, decay: 0.25, pan: [0.25,-0.25].choose
    sleep 1/2.0/2.0
  }
end
...
live_loop :third_voice, sync: :thing do
  with_fx :pitch_shift do
    synth :dark_sea_horn, note: (knit :Gs1, 1, :A1, 1).tick, amp: 0.25, decay: (ring 0.25, 0.25, 0.25/2.0, 0.25).tick(:D), attack: 0.01
    synth :plucked,       note: (knit :Gs2, 1, :A2, 1).tick, amp: 0.8, decay: (ring 0.25, 0.25, 0.25/2.0, 0.25).tick(:D), attack: 0.25
  end
  sleep 2
end
...
with_fx(:reverb, room: 0.9, mix: 0.4, damp: 0.5) do |r_fx|
  live_loop :hollower, sync: :thing do
    with_transpose (knit 0,3,7.0,1).tick(:t) do
      d = (ring
  ...
  with_transpose 12 do
    synth :hollow, note: d2, amp: 16.0
      ...
      with_fx :distortion, mix: 0.5 do
        ...
        if d[0] == [:fs4]
          ...
          n = synth :gpa, note: d[0],  amp: 3.6, release: d[1]*2, attack: 0.0001, wave: 4, cutoff: (ramp *(range 0,130, 1)).tick(:r2)
        end
        ...
        ).tick(:n2), amp: 8.0, release: d[1]/4.0, cutoff: (range 80,100, 5).tick(:c)
    end
    _=:r
    synth :blade, note: (ring
                         ...
                         _, _, _, _,
                         _, _, _, _,
                         ).tick(:n3), amp: 4.5, release: d[1], cutoff: (range 80,100, 5).tick(:c2)
    sleep d[1]
  end
end
end
...
live_loop :beat, sync: :thing do
  with_fx :lpf, cutoff: 130 do
    florish = spread(1,2, rotate: 0).tick(:flo)
    if florish
      sample Fraz[/kick/,[2,2]].tick(:sample), cutoff: 130, amp: 1.0
      sleep 1/2.0/2.0
      sample Fraz[/kick/,[2,2,2,2]].tick(:sample), cutoff: 130, amp: 0.8
      sleep 1/2.0/2.0
      #sample Frag[/kick/,[4,5]].tick(:sample), cutoff: 110, amp: 0.1
      sample Fraz[/kick/,[2,2,2,2]].tick(:sample), cutoff: 130, amp: 0.8
      
      sample Ether[/snare/,[0,0]].tick(:sample), cutoff: 135, amp: 1.0
      sleep 1/2.0
    else
      sample Fraz[/kick/,[4,4,4,4]].tick(:sample2), amp: (knit 1.2, 1, 1.2, 3).tick(:kickmal)
      sleep 1/2.0
      #sample Frag[/kick/,[1,0]].tick(:sample), cutoff: 100+rand, amp: 0.8
      sample Fraz[/kick/,[2,2,2,2]].tick(:sample), cutoff: 130, amp: 0.8
...
live_loop :perc, sync: :thing do
  ...
  tick
  if spread(4,8).look
    # sample CineElec[/hat/].tick(:sample), amp: 0.5
  end
  p = [[4,4],[6,6]].shuffle
