# Loading previous buffer contents. Please wait...["instruments","shaderview","experiments", "log","samples"].each{|f| require "/Users/josephwilk/Workspace/repl-electric/live-coding-space/lib/#{f}.rb"}; _=nil
set_volume! 0.1
_=nil
use_bpm 60
live_loop :bazz, sync: :thing do
  ...
  with_transpose 12 do
    #if b[0].length > 1
    #   synth :gpa, note: b[0][0], amp: 1.0, decay: b[1]*1 , attack: 0.001
  end
  #end
  #if b[0].length == 1
  #synth :dark_sea_horn, note: b[0][0], amp: 1.0, decay: 8*b[1] , attack: 0.001
  #end
  synth :plucked, note: (ring :Fs2, :A2, :CS2, :A2).tick(:bass) , amp: 1.0, decay: 2 , attack: 0.001, cutoff: 90
  sleep 1
end
    ...
    #sample (ring Frag[/coil/, /f#/, 2], nil).tick(:s), rate: 2.0, amp: 8.0 if d[0] == :fs4
      ...
      with_fx :distortion, mix: 0.0 do
        if d[0] == [:fs4]
          ...
          with_transpose 0 do
            n = synth :gpa, note: d[0],  amp: 5.6, release: d[1]*2, attack: 0.0001
          end
        end
        synth :hollow, note: (ring
                              ...
                              ).tick(:n2), amp: 0.0, release: d[1]*10.0, cutoff: (range 80,100, 5).tick(:c)
      end
      _=:r
      synth :blade, note: (ring
                           ...
                           _, _, _, _,
                           _, _, _, _,
                           ).tick(:n3), amp: 1.5, release: d[1], cutoff: (range 80,100, 5).tick(:c2)
      sleep d[1]
    end
  end
end
