...
live_loop :sop do
  ...
  with_fx((knit :reverb,3, :echo, 1).tick(:fx), room: 0.9, mix: 0.8, damp: 0.5, decay: 16.0) do |r_fx|
    with_fx((knit :reverb,3, :none, 1).look(:fx), room: 0.9, mix: 0.8, damp: 0.5, decay: 16.0) do |r_fx|
...
live_loop :vocalm do
  2.times{sync :warm}
  sample Fraz[/coil/,/c#/,1], cutoff: 60, amp: 0.5
end
...
live_loop :beat do
  ...
  with_fx(:reverb, room: 0.9, mix: 0.4, damp: 0.5) do |r_fx|
    if spread(1,2).tick(:d)
      #    sample Mountain[/ice/,0], cutoff: 50, amp: 0.4
    end
  end
  sleep 2
end
...
live_loop :warm do
  notes = ring([:Fs3, :A3, :CS3]).tick
  with_fx(:reverb, room: 0.6, mix: 0.4, damp: 0.5) do |r_fx|
    with_fx(:pitch_shift, time_dis: 0.01) do
      synth :dsaw, cutoff: 40, note: notes[0], amp: 0.1, decay: 4.0
      synth :growl, cutoff: 120, note: notes[0], amp: 0.4, decay: 4.0, release: 3.0
    end
  end
  ...
  7.times{
    ...
    if spread(1,7).tick(:s)
      ...
      synth :hollow, note: (ring :Cs3, :E3).tick(:hnotes),
        decay: 4.0, attack: 1.0, cutoff: 100, release: 4.0
    ...
    s = synth :gpa, note: n.tick(:notes), decay: 1.0, amp: 0.8,wave: 6, note_slide: 0.1
    n.tick(:notes)
    sleep 1
    control s, note: n.tick(:notes)

    if spread(1,8).look(:s)
...
live_loop :bassline do
  with_fx :distortion, distort: (range 0.1,0.4,0.05).tick(:go)  do
    n = (ring
         :E2, :E2, :Gs2, :E2,
         :E2, :Gs2,:E2, :E2,
         :E2, :Gs2,:E2, :E2,
    ...
    #synth :dsaw, note: n, decay: 0.01, amp: 0.4, release: 0.2, cutoff: 35
    #    synth :beep, note: n , decay: 0.01, wave: 6, amp: 0.4, release: 0.2
  end
  sleep 0.25*1
end
