...
live_loop :piano do |p_idx|;with_fx :level, amp: 0.0 do
    ...
    with_fx :reverb do
      ...
      with_fx :level, amp: 0.0  do
        ...
        notes = deg_seq(*%w{:a3 6 _ 6 _  7 _ _ _ 6 _ _ _ 7 _ _ _})

        sleeps = (ring bar/2.0, bar/2.0, bar/2.0, bar/2.0, bar/2.0, bar/2.0)
...
define :play_rolling do |note, direction, room_size, cutoff, detune_factor1, detune_factor2|
  ...
  with_fx :reverb, mix_slide: 0.2 do |r|
    with_fx :lpf, cutoff: 20, cutoff_slide: 20 do |c|
...
live_loop :startzse do |s_idx|; with_fx :level, amp: 0 do
    sync :start
...
live_loop :thebass do
  with_fx :level, amp: 0.0 do
    use_synth :tri
    with_fx :echo, mix: 0.2 do; with_fx :reverb, room: 0.9, room_slide: 2 do |r|; with_fx :distortion, mix: 0.8 do
          sync :start
