...
with_fx :level, amp: 0 do
  ...
  with_fx :reverb, room: 1.0, mix: 1.0 do
    ...
    b4_s = "/Users/josephwilk/.overtone/orchestra/violin/violin_B5_phrase_mezzo-piano_arco-major-trill.wav"
#    with_fx :slicer, phase: bar/8 do
     sample [b4_s].choose, amp: 1, pan: Math.sin((vio_pan))
#    end 
#    sample v_s, amp: 90
  end
end
...
define :beeper do |note, direction, room_size, cutoff, detune_factor1, detune_factor2|
  use_synth :mod_dsaw #:mod_fm # sample v_s, amp: 90
...
with_fx :level, amp: 0 do
  ...
  #n =  [degree(1, :A4, :major)].choose fm

  #sync :start

  4.times do
    sync :bar
    beeper n, @direction == -1 ? 0 : -1, 1, 100, 0, 0.01
    room_size += 1
  end
end
end
...
sample a4_long_s, amp: 40, rate: 1.0
#sample a4_long_s, amp: 40, rate: 0.4
#sample a4_long_s, amp: 40, rate: 0.8
#sample a4_long_s, amp: 40, rate: 1.0
