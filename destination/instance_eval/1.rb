...
live_loop :organ do
  synth :hollow, note: :fs3, attack: 1.0, decay: 4.0, amp: 2.0
  
with_fx  :bitcrusher, bits: 10, sample_rate: 20000, cutoff: 100, mix: 1.0 do #:distortion, feedback: 0.5, mix: 0.0 do
...
#  sample Organ[/_d2_/,0], cutoff: 130, amp: 3.5, pan: Math.sin(vt*13)/1.5
  ...
  synth :dark_ambience, note: (ring :e3, :e4,:fs3).tick, cutoff: 100, decay: 8.0, amp: 2.5, attack: 0.001

  with_fx :slicer, mix: [0.0,0.2].choose do
    with_fx :echo, decay: 8.0 do
      with_fx(:distortion, mix: 0.1, distort: 0.4) do |r_fx|
        with_fx :reverb, room: 1.0 do
          sample Instruments[/cello/, /pianissimo_arco/, /fs2_1|a2_1|cs3_1/].tick, amp: 8
        end
      end
    end
  end
