...
with_fx :level, amp: 0.6 do
  ...
  use_synth :beep
  with_fx :reverb do
    #sample pa_s, amp: 1, rate: (ring -1, 1)[p_idx]

    play degree(1, :A3, :minor), amp: 1.0, attack: 0.001, release: 0.01, decay: 0.1
    sleep bar/2
    play degree(2, :A4, :minor), amp: 1.0, attack: 0.001, release: 0.01, decay: 0.1
    sleep bar/4
    ...
    play degree(1, :A3, :minor), amp: 1.0, attack: 0.001, release: 0.01, decay: 0.1
    sleep bar/2
    play degree(3, :A4, :minor), amp: 1.0, attack: 0.001, release: 0.01, decay: 0.1
    sleep bar/4
    play degree(1, :A4, :minor), amp: 1.0, attack: 0.001, release: 0.01, decay: 0.1   
    sleep bar/4
    play degree(1, :A4, :minor), amp: 1.0, attack: 0.001, release: 0.01, decay: 0.1   
...
with_fx :level, amp: 0.4 do
  ...
  with_fx :reverb do
    ...
    sleep bar
    control e, phase: bar/2 +  0.1
    play degree(4, :A4, :major), amp: 1.0, attack: 0.01, release: 0.09, decay: bar/4
    sleep bar/2
    ...
    sleep bar*2
    play degree(1, :A5, :major), amp: 1.1, attack: 0.001, release: 0.1, decay: bar/2 + 0.05
    sleep bar/4
    with_fx :echo, phase: bar/8 do
      play degree(1, :A3, :major), amp: 1.0, attack: 0.01, release: 1.0, decay: dice(6) > 3 ? bar*4 : bar/2 + 0.05
      with_fx :distortion, mix: 0.01 do
...
#chord_degree(1, :A2)[0],
#chord_degree(1, :A2)[0]
)[n_idx], release: (ring bar*4, bar*4, bar*4, bar*5)[n_idx], attack: 0.1
end
end
end
end
...
live_loop :beep do |b_idx|
with_fx :level, amp: 1 do
  sync :beep2
  #-10.0, -5.0
  # degree(6, :A2, :major)
  #n = [degree(4, :A3, :major)].choose
  n =  (ring degree(1, :A3, :major),degree(4, :A3, :major))[b_idx] #fm
...
live_loop :beep2 do |n_idx|
notes = (ring degree(5, :A3, :major), degree(6, :A3, :major))[n_idx]
